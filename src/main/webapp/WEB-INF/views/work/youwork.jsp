...
<%--            </select>--%>
    ...
    <div class="table">
        <div class="row head">
            ...
            <div class="col">
                <span data-sort="workername">수신자<i class="fa-solid ${sort:getSortIcon(param.sort, param.dir, 'workername', 'writedate', 'desc')}"></i></span>
            </div>
            <div class="col">
    ...
    <c:if test="${not empty list}">
        <div class="paging" style="height: 50px; line-height: 50px">
            <c:if test="${paging.prev}"><a href="yourwork?page=${paging.beginPage-1}&type=${type}&key=${key}&sort=${sort}&dir=${dir}">Prev</a></c:if>

            <c:forEach begin="${paging.beginPage}" end="${paging.endPage}" var="index">
                <a href="yourwork?page=${index}&type=${type}&key=${key}&sort=${sort}&dir=${dir}"<c:if
                        test="${index == paging.page}"> style="color: red;"</c:if>>${index}</a>
            </c:forEach>

            <c:if test="${paging.next}"><a href="yourwork?page=${paging.endPage+1}&type=${type}&key=${key}&sort=${sort}&dir=${dir}">Next</a></c:if>
        </div>
    </c:if>
</section>
